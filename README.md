...
- The official Unity CLI and `com.unity.pipeline` versions supported by
  `VMUnityPipeline`.
- `com.vm233.unity-automation` 0.6.10, including retained Editor window capture, complete test results across assembly reloads, explicit Profiler capture retirement, cooperative job cancellation, owned command effects, in-place PNG resize and exact inherited Prefab property reversion.
