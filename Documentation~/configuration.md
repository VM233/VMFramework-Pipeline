...
| `validate-game-prefabs` | Shared issue limit | Scans every discoverable GamePrefab Wrapper, compares each exact config object with the runtime `GlobalSettingCollector` provider graph, and verifies each readable `IPrefabProvider.Prefab` name against the GamePrefab id using its declared id suffix. Unreachable configs fail with `unregistered_game_prefab`; name mismatches fail with `prefab_name_id_mismatch` and expose the Prefab path/name plus derived expected id. `maxIssues` bounds only returned records, while aggregate counts and `passed` always cover the complete bounded scan. |
...
- VM Unity Automation 0.1.7 preserves the declared owner package, `inputSchema`, and `outputSchema`
